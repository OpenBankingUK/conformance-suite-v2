...
- Defence-in-depth loopback guard on the conformance REST API. All three endpoints (`POST /api/runs/`, `GET /api/runs/<id>/`, `GET /api/runs/<id>/result/`) now reject any request whose `REMOTE_ADDR` is not an IPv4/IPv6 loopback address (`127.0.0.0/8`, `::1`) with HTTP 403. The guard backstops the Phase 1 deployment requirement (publish the Docker port to 127.0.0.1) so a misconfigured `-p 0.0.0.0:8000` does not silently expose the unauthenticated API — important because `create_run` can trigger outbound HTTP requests using mounted mTLS credentials. `X-Forwarded-For` is deliberately not honoured; trusting forwarded headers without a vetted proxy chain would itself be a security bug. Set `CONFORMANCE_API_ALLOW_NON_LOCAL=true` to opt out (e.g. when fronting the API with an authenticated reverse proxy).
- Bounded history for the in-memory `RunStore`: only the most recent 10 terminal (`completed`/`failed`) run records are retained, with the oldest evicted on each new `create_run`. Active (`pending`/`running`) records are never pruned. Prevents unbounded memory growth in long-lived dev containers or persistent processes that handle many runs over time.
- REST API for headless/CI usage (`POST /api/runs/`, `GET /api/runs/<id>/`, `GET /api/runs/<id>/result/`). Unauthenticated, designed for local Docker deployment. The primary access control is binding the published Docker port to 127.0.0.1; an application-level loopback guard (see above) backstops this. Supports starting a conformance run with inline config + optional manifest, polling run status, and retrieving the structured result. Phase 1 enforces one active run at a time (409 Conflict on concurrent attempts). Runs execute asynchronously in a background thread; the POST returns immediately with the run ID and pending status. CSRF is exempt (no browser session involved — designed for programmatic/CI access per PRD). Implements PRD OBL Engineering Story #5: _"the engine to expose a REST API (unauthenticated for local Docker, bound to localhost), so that the tool can be called programmatically from scripts or CI pipelines"_.
...
- API `create_run` now returns HTTP 400 for both invalid JSON (`json.JSONDecodeError`) and invalid UTF-8 (`UnicodeDecodeError`) request bodies. `json.loads` decodes bytes as UTF-8 internally and raises `UnicodeDecodeError` before `JSONDecodeError` gets a chance, so both must be caught at the parse boundary. Behaviour for the JSON-decode path is unchanged; the UTF-8 path was previously unhandled.
