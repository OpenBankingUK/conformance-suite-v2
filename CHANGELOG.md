...
- `skipped` outcome state for steps that could not run because a prerequisite setup step failed (PRD: "Result Outcomes"). The v0 executor now emits the configured `followUp` as a `skipped` step (with message `Skipped because primary step '<id>' failed` and the primary URL) instead of silently dropping it when the primary step fails. `SmokeCheckResult` summaries gain a `skipped` count alongside `passed` and `failed`. Aggregate status is `failed` iff at least one step failed — `skipped` steps are informational and do not on their own fail a run.
