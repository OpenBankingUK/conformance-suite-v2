...
- **Private function and method docstrings**: Every private (`_`-prefixed) module-level function and method must also have a Google-style docstring. This enables IDE hover documentation and makes the codebase navigable without reading entire function bodies. Trivial private helpers (fewer than ~10 lines with an obvious signature) may use a one-line summary; `Args:`/`Returns:` blocks are optional on these. `Raises:` is still required when the helper raises exceptions directly. Note: ruff's Google pydocstyle convention does not enforce `D1xx` on private names, so this rule is enforced by code review rather than the linter.
