...
| ID | Decision | Rationale |
| --- | --- | --- |
| `TPA-049` | AIS has independent requirements and test-definition catalogues for Read/Write `3.1.11` and `4.0.1`. Each catalogue exposes 12 participant-selectable resource capabilities and all 29 operations from the matching Account Information OpenAPI document. Resource capabilities explicitly depend on account-access consent, and non-account resources also depend on account discovery. | Preserves the profile's consent-first flow and requirement that `GET /accounts` is the first resource call without encoding those dependencies in array order, legacy setup cases, or runner behavior. |
| `TPA-050` | The shared vocabulary adds capability dependencies, `DELETE`, timezone-aware and timezone-less ISO 8601 date-time inputs, query-parameter input bindings, authorization and path-parameter request generation, multi-status HTTP assertions, and response-header equality with the originating request. v3.1.11 transaction filters use RFC 3339 values; v4.0.1 uses the specification's timezone-less form. These forms are protocol-neutral and are carried unchanged into generated execution manifests. | AIS provides concrete specification cases that the PIS-only vocabulary could not represent: consent revocation, version-specific transaction date filters, consent-bound permission contexts, invalid resource identifiers, specification-permitted alternative error statuses, and `x-fapi-interaction-id` playback. |
| `TPA-051` | AIS requirements bind every endpoint to the content-addressed OpenAPI operation and cite the Account and Transaction API Profile for consent, permission, authorization, and request-order obligations. Predefined transaction date inputs remain logical participant values; test definitions own their query-parameter bindings. | Keeps OpenAPI technical inventory, normative obligations, participant intent, and executable request construction separate while retaining exact traceability to both authoritative sources. |
