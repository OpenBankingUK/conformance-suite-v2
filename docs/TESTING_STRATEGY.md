...
- **Endpoint-first readiness reporting** — target-driven runs must populate `readinessReport` from compiled catalogue policy and coverage, including catalogue hash, omitted mandatory endpoints/operations, per-resource-group readiness/certification status, and DCR non-certifying status. See `tests/test_results.py`, `tests/test_plan_executor.py`, and `tests/test_ui_views.py`.
