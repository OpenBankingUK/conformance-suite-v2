...
| Require status checks to pass | **Ready to enable** — CI workflows now exist (`ci.yml`, `e2e.yml`). Enable after the first successful run on `develop` so GitHub recognises the check contexts (see TODO below) |
...
- [ ] **Enable required status checks** — CI workflows are now committed (`ci.yml` with jobs `Lint & Type Check`, `Unit & Integration Tests`, `Docker Build`; `e2e.yml` with job `End-to-End Conformance Tests`). After the first successful run on `develop`, enable the "Require status checks to pass" rule and add these check contexts
