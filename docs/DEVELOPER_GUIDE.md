...
| `ob-read-write` | `v4.0.1` | `fapi1-advanced` | `pis-domestic-payment-starter` | Sandbox/model-bank-only PIS domestic-payment starter: discovery, JWKS, client-credentials token, consent creation, manual PSU authorisation, consent read-back, funds confirmation, domestic payment submission, and payment read-back. | `oauth.clientId`, `oauth.redirectUri`, `oauth.resourceBaseUrl`, `fapiSigning.*` |
| `ob-read-write` | `v4.0` | `fapi1-advanced` | `pis-fcs-legacy-benchmark` | PIS prior-FCS parity benchmark: discovery, JWKS, client-credentials token, dual domestic-payment consent variants, manual PSU authorisation, token exchange, consent read-back, funds confirmation, domestic payment submission, payment read-back, and 21 conditional scheduled/standing/international rows that auto-select when required test values are present. | `oauth.clientId`, `oauth.redirectUri`, `oauth.resourceBaseUrl`, `fapiSigning.*` |
...
The custom test-value model now splits responsibilities across three layers: suite manifests define `testValues.baseline` plus `generatedKeys` and `allowedCustomKeys`, participant configs provide `testData.values`, and `RunConfigurationCompiler` assembles the compiled `RunConfiguration` used at execution time. The compiler normalises same-as-baseline values away, so result evidence only reports baseline-delta impact for genuine customisation. This keeps certification strict while still allowing exploratory runs.

The PIS domestic-payment starter uses that model with Ozone-compatible `testValues.baseline` defaults and participant `testData.values` deltas. It still resolves payment payload fields through `${testValues.<key>}` placeholders and records conditional-step `selectionMetadata`, but plan previews now explain whether a row is selected because it has genuine baseline deltas or remains aligned with the suite baseline. Result evidence and execution logs carry masked baseline-delta impact rather than profile/override vocabulary. The detached-JWS allowlist for this starter stays narrow and endpoint-specific, covering only the domestic-payment consent/write paths and the documented negative signing checks. The headless `psu-authorization-signing-negative` row declares an expected `HTTP 400` rejection; Ozone-style non-callback redirect rejections are also treated as expected only for that explicit negative signing case.

The `pis-fcs-legacy-benchmark` suite uses the same oauth and signing inputs as `pis-domestic-payment-starter`. It preserves all 29 legacy PIS IDs, adds `OB-400-DOP-100300` as the debtor-account consent variant, and marks scheduled/standing/international rows as conditional via `selectionMetadata` so they default-select when prerequisite `testValues` are available. The committed example `config/model-bank-pis-fcs-legacy-benchmark-example.json` intentionally keeps participant fields as placeholders and provides non-secret `testValues` overrides for scheduled/standing/international gating coverage. The suite remains `certificationCoverage: partial` and is a prior-FCS parity benchmark rather than a certification claim.

Recommended runnable flow for this benchmark is:

1. Copy the committed example to a private local config path and replace placeholder OAuth/FAPI-signing/Open Banking fields.
2. Keep certificate/key files local (for example under `local-config/`) and out of version control.
3. Run with CLI (`uv run python main.py <your-local-config>.json`) or preview first in `/plan/` with a blank manifest textarea and config-selected `testSuite`.
4. Inspect both `resultOutputPath` (result JSON) and `executionLogPath` (NDJSON) after each run to confirm selected/deselected conditional rows and masked evidence.

Offline CI smoke tests should stay network-free (`make test` / `make check`). Optional live Ozone validation is manual and local-only; use `CONFORMANCE_DEVELOPER_MODE=true` only when debugging engine internals and never for shared infrastructure or real participant data.
