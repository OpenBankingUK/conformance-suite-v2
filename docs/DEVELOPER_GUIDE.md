...
The PIS domestic-payment starter also needs `oauth.clientId`, `oauth.redirectUri`, `oauth.resourceBaseUrl`, and `fapiSigning.*`. Its manifest uses the new `testValueProfiles` contract to supply Ozone-demo defaults plus a synthetic fallback, resolves payment payload fields through `${testValues.<key>}` placeholders, and records conditional-step `selectionMetadata` so plan previews can explain whether a row was selected by the default profile, by participant overrides in `testValues.profile` / `testValues.overrides`, or left deselected because required keys were missing. Result evidence and execution logs carry masked/non-secret profile metadata rather than raw secret material. The detached-JWS allowlist for this starter stays narrow and endpoint-specific, covering only the domestic-payment consent/write paths and the documented negative signing checks. The headless `psu-authorization-signing-negative` row declares an expected `HTTP 400` rejection; Ozone-style non-callback redirect rejections are also treated as expected only for that explicit negative signing case.
