"""Deterministic DCR protocol fixtures, in-process and over local mTLS.

The Open Banking DCR 3.4 protocol state machine lives in
:class:`DcrProtocolService` and is transport independent: tests that exercise
protocol or orchestration behaviour drive it through an in-process
:class:`httpx.MockTransport` at the client boundary, with no sockets and no
TLS handshakes. :class:`DcrTestService` adds a real loopback HTTPS listener on
top of the same state machine and is reserved for tests where transport, TLS,
mTLS, certificates, or the product's own client configuration are themselves
under test.
"""
...
_IN_PROCESS_BASE_URL = "https://dcr-fixture.localhost"
"""Stable origin advertised by the in-process protocol service."""

_SHUTDOWN_POLL_INTERVAL_SECONDS = 0.02
"""Listener poll interval, so stopping the loopback service is not a fixed 0.5s wait."""
...
@dataclass(frozen=True)
class DcrFixtureMaterials:
    """Immutable mTLS and protocol material shared across DCR fixtures.

    Generating RSA keys and certificates dominates DCR fixture cost, and none
    of this material is mutated by a test, so a single set is generated once
    and reused. Mutable protocol state lives on
    :class:`DcrProtocolService` and is created fresh per test.
    """

    tls: DcrTlsMaterials
    protocol: DcrProtocolMaterials
...
class DcrProtocolService:
    """Transport-independent deterministic implementation of the DCR protocol.

    The state machine is driven directly by :meth:`dispatch`, so tests can
    exercise Open Banking DCR 3.4 protocol and orchestration behaviour through
    an in-process :class:`httpx.MockTransport` without opening a socket.
    """

    def __init__(
        self,
        materials: DcrFixtureMaterials,
        *,
        base_url: str = _IN_PROCESS_BASE_URL,
        ...
        """Create a service with empty mutable state over shared credentials.

        Args:
            materials: Immutable mTLS and protocol material to serve.
            base_url: Origin this service advertises in discovery metadata and
                registration URLs.
        ...
        self.tls = materials.tls
        self.protocol = materials.protocol
        ...
        self._mtls_verified = False

    @property
    def base_url(self) -> str:
        """Return the origin this service advertises.

        Returns:
            HTTPS origin used to build every absolute protocol URL.
        """
        return self._base_url

    def transport(self) -> httpx.MockTransport:
        """Build an in-process transport that routes requests into the protocol.

        Returns:
            Transport that serialises responses exactly as the loopback
            listener does, without a socket or TLS handshake.
        """
        return httpx.MockTransport(self._handle_in_process)

    def client(self) -> httpx.Client:
        """Build an in-process HTTP client bound to this service.

        Returns:
            Caller-owned synchronous HTTP client that never leaves the process.
        """
        return httpx.Client(base_url=self.base_url, transport=self.transport(), trust_env=False, timeout=3.0)

    def _handle_in_process(self, request: httpx.Request) -> httpx.Response:
        """Serve one in-process request through the protocol state machine.

        Args:
            request: Outbound client request intercepted before transport.

        Returns:
            Deterministic JSON response with the same body bytes the loopback
            listener would frame.
        """
        ...
        return httpx.Response(response.status_code, headers=headers, content=_serialize_body(response.body))
    ...
    def _dispatch(
        ...
        """Route one request through the deterministic protocol state machine.
        ...
        with self._lock:
            ...
            self._events.append(
                DcrServiceEvent(
                    ...
                    mtls_verified=self._mtls_verified,
...
class DcrTestService(DcrProtocolService, AbstractContextManager["DcrTestService"]):
    """Local mTLS HTTPS listener in front of the DCR protocol state machine.

    Reserved for tests where transport, TLS, mTLS, certificates, or the
    product's own HTTP client configuration are under test. Protocol and
    orchestration behaviour should drive :class:`DcrProtocolService` in
    process instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Create an unstarted loopback service over shared credentials.
        ...
        super().__init__(
            ...
            response_omissions=response_omissions,
            response_overrides=response_overrides,
        )
        self._server: ThreadingHTTPServer | None = None
        self._thread: threading.Thread | None = None
        # The listener sets ``ssl.CERT_REQUIRED``, so every dispatched request
        # has already completed a verified mutual-TLS handshake.
        self._mtls_verified = True

    @property
    def base_url(self) -> str:
        """Return the running listener's HTTPS origin.
    ...
    def start(self) -> Self:
        ...
        self._thread = threading.Thread(
            ...
            args=(_SHUTDOWN_POLL_INTERVAL_SECONDS,),
            name="dcr-test-service",
            daemon=True,
        )
        self._thread.start()
        return self
    ...
    def close(self) -> None:
        ...
        Raises:
            RuntimeError: If the listener thread does not exit promptly, so a
                leaked worker fails the owning test instead of the next one.
        """
        server = self._server
        thread = self._thread
        if server is None:
        ...
        if thread is not None:
            ...
            if thread.is_alive():
                raise RuntimeError("DCR test service listener did not stop")
    ...
    def _serve_http(self, handler: BaseHTTPRequestHandler) -> None:
        ...
        payload = _serialize_body(response.body) if response.body is not None else b""
...
def _serialize_body(body: JsonObject) -> bytes:
    """Serialize a protocol response body to stable JSON bytes.

    Args:
        body: Deterministic JSON response object.

    Returns:
        Compact, key-sorted UTF-8 encoding shared by both transports.
    """
...
def create_dcr_fixture_materials(root: Path) -> DcrFixtureMaterials:
    """Generate the immutable mTLS and protocol material for DCR fixtures.

    Args:
        root: Directory receiving ephemeral PEM and JOSE key files.

    Returns:
        Material safe to share across tests because nothing mutates it.
    """
    root.mkdir(parents=True, exist_ok=True)
    return DcrFixtureMaterials(
        tls=_create_tls_materials(root / "tls"),
        protocol=_create_protocol_materials(root / "protocol"),
    )
...
def running_dcr_test_service(
    ...
    *,
    ...
    Yields:
        Running service instance.
    """
    with DcrTestService(materials, management_methods=management_methods) as service:
        yield service
