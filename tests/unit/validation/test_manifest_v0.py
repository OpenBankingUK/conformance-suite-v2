"""Parsing of legacy v0 manifest documents, including follow-up requests."""
...
from tests.support.manifest_documents import (
    v0_assertion_configs,
    v0_first_test,
    v0_follow_up_config,
    v0_manifest,
    v0_request_config,
...
def test_load_manifest_returns_typed_manifest(tmp_path: Path) -> None:
    ...
    manifest_path.write_text(json.dumps(v0_manifest()), encoding="utf-8")
...
def test_parse_manifest_accepts_valid_minimal_discovery_manifest() -> None:
    raw_manifest = v0_manifest()
    v0_first_test(raw_manifest).pop("followUp")
...
def test_parse_manifest_rejects_non_https_request_url() -> None:
    raw_manifest = v0_manifest()
    v0_request_config(raw_manifest)["url"] = "http://example.com/.well-known/openid-configuration"
...
def test_parse_manifest_rejects_malformed_https_request_url(url: str) -> None:
    raw_manifest = v0_manifest()
    v0_request_config(raw_manifest)["url"] = url
...
def test_parse_manifest_rejects_non_get_request_method() -> None:
    raw_manifest = v0_manifest()
    v0_request_config(raw_manifest)["method"] = "POST"
...
def test_parse_manifest_rejects_unsupported_assertion_type() -> None:
    raw_manifest = v0_manifest()
    v0_assertion_configs(raw_manifest).append({"type": "token_claim", "claim": "iss"})
...
def test_parse_manifest_rejects_unsupported_json_field_rule() -> None:
    raw_manifest = v0_manifest()
    json_field_assertion = cast("dict[str, JsonValue]", v0_assertion_configs(raw_manifest)[1])
...
def test_parse_manifest_rejects_invalid_http_status_code(expected: JsonValue) -> None:
    raw_manifest = v0_manifest()
    http_status_assertion = cast("dict[str, JsonValue]", v0_assertion_configs(raw_manifest)[0])
...
def test_parse_manifest_rejects_unsupported_follow_up_shape() -> None:
    raw_manifest = v0_manifest()
    v0_follow_up_config(raw_manifest)["type"] = "token_endpoint"
...
def test_parse_manifest_rejects_unsupported_follow_up_url_source() -> None:
    raw_manifest = v0_manifest()
    v0_follow_up_config(raw_manifest)["urlSource"] = "response.body.issuer"
...
def test_parse_manifest_rejects_null_follow_up() -> None:
    raw_manifest = v0_manifest()
    v0_first_test(raw_manifest)["followUp"] = None
...
def test_parse_manifest_rejects_non_get_follow_up_request_method() -> None:
    raw_manifest = v0_manifest()
    follow_up_request = cast("dict[str, JsonValue]", v0_follow_up_config(raw_manifest)["request"])
...
def test_parse_v0_manifest_always_has_partial_coverage() -> None:
    ...
    manifest = parse_manifest(v0_manifest())
