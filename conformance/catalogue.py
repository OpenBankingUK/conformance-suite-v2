...
"""Protocol-neutral execution-step kinds represented by catalogue definitions."""

type CatalogueGateAbsenceBehavior = Literal["exclude", "skip"]
"""Result policy when a catalogue trace group is gated by unselected endpoints."""
...
_CANONICAL_OBL_DCR_FAMILY = "OBL_DCR"
"""Canonical PRD family value for Open Banking UK DCR plans."""
...
@dataclass(frozen=True)
class CatalogueConfigurationRequirement:
    """Specification-owned configuration needed by catalogue execution.

    Attributes:
        config_path: Canonical plan path containing the configuration value.
        input_type: Expected value type at the canonical plan boundary.
        label: Human-readable description of the required configuration.
        required: Whether execution requires the value.
        sensitive: Whether evidence must suppress the value.
        endpoint_refs: Endpoint selections that make the requirement applicable.
            An empty tuple means the requirement applies to every compiled scope.
    """
...
@dataclass(frozen=True)
class CatalogueRuntimeCapability:
    """Runtime capability policy declared by a catalogue.

    Attributes:
        capability_id: Stable identifier for the runtime capability.
        label: Human-readable capability name.
        description: Normative execution constraint represented by the policy.
        supported_values: Values the runtime is required to execute.
        unsupported_values: Values the runtime must reject explicitly.
        generated_dependency: Whether the capability is generated by compilation
            rather than participant-selectable endpoint scope.
    """
    ...
    generated_dependency: bool = False


@dataclass(frozen=True)
class CatalogueProvenance:
    """Pinned source provenance for a versioned catalogue.

    Attributes:
        repository: Source repository used as the parity baseline.
        release: Released source version used by the catalogue.
        commit: Immutable source commit used by the catalogue.
        source_paths: Source paths that define or adapt the covered behaviour.
        references: Stable reference identifiers and their pinned URLs.
    """
...
@dataclass(frozen=True)
class CatalogueTraceGroup:
    """Generic parent-group trace metadata for related catalogue cases.

    Attributes:
        group_id: Stable identifier shared by the grouped test cases.
        name: Human-readable group name.
        intent: Coverage intent preserved for result grouping.
        source_symbol: Pinned legacy source symbol defining the group.
        normative_reference_ids: Stable normative references for the group.
        required_endpoint_refs: Endpoint selections required by the whole group.
        absence_behavior: How later result generation should represent a group
            whose required endpoint selection is absent.
    """
    ...
    intent: str
    source_symbol: str
    normative_reference_ids: tuple[str, ...]
    required_endpoint_refs: tuple[EndpointRef, ...]
    absence_behavior: CatalogueGateAbsenceBehavior = "exclude"


@dataclass(frozen=True)
class CatalogueExecutionStep:
    """Ordered protocol-neutral step in an executable catalogue case.

    Attributes:
        step_id: Stable traceability identifier unique in the catalogue.
        definition_id: Stable operation definition referenced by the step.
        name: Human-readable step name.
        kind: Execution adapter operation family.
        behavior: Pinned description of the required operation.
        legacy_operation: Pinned legacy builder operation for parity evidence.
        state_inputs: Scenario-local state values consumed by the step.
        state_outputs: Scenario-local state values produced by the step.
        expected_status: Locked HTTP status expected by this step, when any.
        sensitive: Whether step evidence contains secret or bearer material.
        variant: Optional negative or alternate request variant.
        deviation_id: Approved parity-ledger deviation applied by the step.
    """

    step_id: str
    definition_id: str
    name: str
    kind: CatalogueExecutionStepKind
    behavior: str
...
class CatalogueTestCase:
    ...
    Attributes:
        ...
        trace_group: Optional generic parent-group trace metadata.
        execution_steps: Ordered protocol-neutral execution steps for adapters
            that require more than the shared JSON HTTP request model.
        state_inputs: Scenario-local state required by this case.
        state_outputs: Scenario-local state produced by this case.
        expected_http_statuses: Locked HTTP status outcomes for the case.
        gate_absence_behavior: How later result generation should represent this
            case when its endpoint applicability gate is not selected.
    """
    ...
    trace_group: CatalogueTraceGroup | None = None
    execution_steps: tuple[CatalogueExecutionStep, ...] = ()
    state_inputs: tuple[str, ...] = ()
    state_outputs: tuple[str, ...] = ()
    expected_http_statuses: tuple[int, ...] = ()
    gate_absence_behavior: CatalogueGateAbsenceBehavior = "exclude"
...
class TestCatalogue:
    ...
    Attributes:
        ...
        configuration_requirements: Canonical configuration requirements owned
            by the specification and consumed by its execution adapter.
        runtime_capabilities: Runtime behavior and generated dependency policies.
        provenance: Optional pinned source provenance for imported coverage.
    """
    ...
    configuration_requirements: tuple[CatalogueConfigurationRequirement, ...] = ()
    runtime_capabilities: tuple[CatalogueRuntimeCapability, ...] = ()
...
class PlanDocumentEndpoint:
    ...
    Attributes:
        ...
        required: Whether the specification requires the direct endpoint.
        locked: Whether participant selection of the direct endpoint is locked.
    """
...
class PlanDocumentV2:
    ...
    Attributes:
        ...
        endpoints: Direct endpoint scope for specifications without resource
...
def plan_document_to_json_object(document: PlanDocumentV2) -> JsonObject:
    ...
    if document.specification == _V2_READ_WRITE_SPECIFICATION:
        ...
        exported["businessTestData"] = _business_test_data_for_export(document)
    else:
        exported["endpoints"] = [_canonical_direct_endpoint_for_export(endpoint) for endpoint in document.endpoints]
        if document.dynamic_client_registration:
            exported["dynamicClientRegistration"] = {
                key: _copy_json_value(value) for key, value in document.dynamic_client_registration.items()
            }
    return exported
...
def supported_plan_document_boundaries(catalogues: Iterable[TestCatalogue]) -> tuple[PlanDocumentBoundary, ...]:
    ...
    """
    available_keys = {catalogue.key for catalogue in catalogues}
    boundaries: list[PlanDocumentBoundary] = []
...
def catalogue_areas_for_plan_document_boundary(
    ...
    Raises:
        CatalogueError: If the boundary is unsupported or has no backing
            catalogue areas.
    """
    _definition, version_definition = _specification_binding_for_plan_boundary(boundary)
    candidates = tuple(
        catalogue
...
def compile_test_plan(catalogue: TestCatalogue, spec: TestPlanSpec) -> CompiledTestPlan:
    ...
    skipped_cases = tuple(
        ...
        if test_case.test_case_id not in selected_ids
        and base_decisions[test_case.test_case_id].reason == "no matching implemented endpoint"
        and (
            test_case.gate_absence_behavior == "skip"
            or test_case.trace_group is not None
            and test_case.trace_group.absence_behavior == "skip"
        )
    )
    traceability = CompilerTraceability(
...
def _compile_plan_document_v2(document: PlanDocumentV2, catalogues: Iterable[TestCatalogue]) -> CompiledTestPlan:
    ...
    if not implemented_endpoints:
        scope_location = "resourceGroups" if document.resource_groups else "endpoints"
        raise CatalogueError(f"testPlan.{scope_location} must select at least one endpoint")
...
def _internal_catalogue_version_for_plan_boundary(boundary: PlanDocumentBoundary) -> str:
    ...
    """
    _definition, version_definition = _specification_binding_for_plan_boundary(boundary)
    return version_definition.catalogue_version


def _specification_binding_for_plan_boundary(
    boundary: PlanDocumentBoundary,
) -> tuple[SpecificationDefinition, SpecificationVersionDefinition]:
    """Resolve registry metadata for a shared plan boundary.

    Args:
        boundary: User-facing scheme/specification/version boundary.

    Returns:
        Specification definition and version-specific catalogue binding.

    Raises:
        CatalogueError: If the boundary is unsupported.
    """
    try:
        return specification_for_boundary(boundary.scheme, boundary.specification, boundary.version)
    ...
    if document.endpoints:
        return tuple(
            ImplementedEndpoint(
                ...
                path=endpoint.path,
                resource_group=document.specification,
...
def _aggregate_catalogue_provenance(
    compiled_plans: tuple[CompiledTestPlan, ...],
) -> CatalogueProvenance | None:
    """Retain unambiguous pinned provenance while aggregating catalogue areas.

    Args:
        compiled_plans: Per-catalogue compiler outputs.

    Returns:
        The sole shared provenance value, or ``None`` when no provenance exists.

    Raises:
        CatalogueError: If multiple catalogue areas carry different provenance.
    """
    provenance = tuple(
        compiled_plan.traceability.provenance
        for compiled_plan in compiled_plans
        if compiled_plan.traceability.provenance is not None
    )
    if not provenance:
        return None
    first = provenance[0]
    if any(candidate != first for candidate in provenance[1:]):
        raise CatalogueError("Cannot aggregate catalogue areas with different source provenance")
    return first
...
def _parse_canonical_plan_document(spec: Mapping[str, JsonValue]) -> PlanDocumentV2:
    ...
    return PlanDocumentV2(
        ...
        resource_groups=(
            _parse_canonical_resource_groups(spec, location="testPlan") if definition.uses_resource_groups else ()
        ),
        ...
        endpoints=(
            () if definition.uses_resource_groups else _parse_canonical_direct_endpoints(spec, location="testPlan")
        ),
        dynamic_client_registration=MappingProxyType(
            _copy_json_mapping(_required_object(spec, "dynamicClientRegistration", location="testPlan"))
...
def _parse_canonical_specification(
...
) -> tuple[PlanDocumentBoundary, SpecificationDefinition, SecurityProfile]:
    ...
    Returns:
        Compiler boundary, specification definition, and security-profile value.
    ...
    if not definition.uses_resource_groups:
        scheme = _required_string(raw_specification, "scheme", location="testPlan.specification")
        name = _required_string(raw_specification, "name", location="testPlan.specification")
        if scheme != definition.scheme:
...
def _validate_canonical_scope_shape(
    raw_plan: Mapping[str, JsonValue],
    *,
    definition: SpecificationDefinition,
) -> None:
    """Enforce the family-owned canonical plan scope shape.

    Args:
        raw_plan: Raw canonical test-plan object.
        definition: Registry definition selected by the family discriminator.

    Raises:
        CatalogueError: If resource-group and direct-endpoint fields do not
            match the selected family.
    """
    if definition.uses_resource_groups:
        if "endpoints" in raw_plan:
            raise CatalogueError("testPlan.endpoints is forbidden for OBL_READ_WRITE")
        if "dynamicClientRegistration" in raw_plan:
            raise CatalogueError("testPlan.dynamicClientRegistration is forbidden for OBL_READ_WRITE")
        if "resourceGroups" not in raw_plan:
            raise CatalogueError("testPlan.resourceGroups is required for OBL_READ_WRITE")
        return
    if "resourceGroups" in raw_plan:
        raise CatalogueError("testPlan.resourceGroups is forbidden for OBL_DCR")
    if "businessTestData" in raw_plan:
        raise CatalogueError("testPlan.businessTestData is forbidden for OBL_DCR")
    if "endpoints" not in raw_plan:
        raise CatalogueError("testPlan.endpoints is required for OBL_DCR")
...
def _parse_canonical_client_auth_method(value: JsonValue) -> None:
    ...
    if value not in {"private_key_jwt", "tls_client_auth", "client_secret_jwt", "client_secret_basic"}:
        raise CatalogueError(
            "testPlan.securityEnvironment.clientAuthMethod must be one of: client_secret_basic, "
...
def _parse_canonical_resource_groups(
    ...
    *,
    ...
    if not raw_groups:
        raise CatalogueError(f"{location}.resourceGroups must contain at least one item")
...
def _parse_canonical_direct_endpoints(
    raw_plan: Mapping[str, JsonValue],
    *,
    location: str,
) -> tuple[PlanDocumentEndpoint, ...]:
    """Parse and validate direct Open Banking DCR endpoint scope.
    ...
    Returns:
        Direct DCR endpoint selections in document order.

    Raises:
        CatalogueError: If endpoint declarations are missing, duplicated, or
            violate the mandatory POST and optional management-operation model.
    """
    raw_endpoints = _required_object_array(raw_plan, "endpoints", location=location)
    seen_refs: set[EndpointRef] = set()
    endpoints = tuple(
        _parse_canonical_direct_endpoint(
            raw_endpoint,
            location=f"{location}.endpoints[{index}]",
            seen_refs=seen_refs,
        )
        for index, raw_endpoint in enumerate(raw_endpoints)
    )
    registration_ref = EndpointRef(method="POST", path="/register")
    if registration_ref not in seen_refs:
        raise CatalogueError(f"{location}.endpoints must include required locked endpoint POST /register")
    return endpoints


def _parse_canonical_direct_endpoint(
    raw_endpoint: Mapping[str, JsonValue],
    *,
    location: str,
    seen_refs: set[EndpointRef],
) -> PlanDocumentEndpoint:
    """Parse one direct Open Banking DCR endpoint selection.

    Args:
        raw_endpoint: Raw endpoint JSON object.
        location: Dot-path location string used in error messages.
        seen_refs: Endpoint references already present in the direct scope.

    Returns:
        Validated direct endpoint selection.

    Raises:
        CatalogueError: If the endpoint is unknown, duplicated, or carries
            participant-editable required/locked metadata.
    """
    _reject_unknown_keys(
        raw_endpoint,
        allowed_keys={"method", "path", "operationId", "required", "locked"},
        location=location,
    )
    ...
    endpoint_policy = {
        EndpointRef(method="POST", path="/register"): (True, True),
        EndpointRef(method="GET", path="/register/{ClientId}"): (False, False),
        EndpointRef(method="PUT", path="/register/{ClientId}"): (False, False),
        EndpointRef(method="DELETE", path="/register/{ClientId}"): (False, False),
    }
    expected_flags = endpoint_policy.get(endpoint_ref)
    if expected_flags is None:
        raise CatalogueError(f"{location} is not a selectable Open Banking DCR 3.4 endpoint: {method} {path}")
    if endpoint_ref in seen_refs:
        raise CatalogueError(f"{location} duplicates direct endpoint {method} {path}")
    seen_refs.add(endpoint_ref)
    required = _required_boolean(raw_endpoint, "required", location=location)
    locked = _required_boolean(raw_endpoint, "locked", location=location)
    if (required, locked) != expected_flags:
        expected_required, expected_locked = expected_flags
        raise CatalogueError(
            f"{location} must set required={str(expected_required).lower()} and locked={str(expected_locked).lower()}"
        )
...
def _required_boolean(raw_object: Mapping[str, JsonValue], key: str, *, location: str) -> bool:
    """Extract a required boolean from a JSON object.
    ...
    Raises:
        CatalogueError: If the field is absent or is not a boolean.
    """
    value = raw_object.get(key)
    if not isinstance(value, bool):
        raise CatalogueError(f"{location}.{key} must be a boolean")
    return value
...
def _canonical_family_for_plan_document(document: PlanDocumentV2) -> str:
    ...
    if document.scheme == _V2_OPEN_BANKING_UK_SCHEME and document.specification == "dynamic-client-registration":
        return _CANONICAL_OBL_DCR_FAMILY
...
def _canonical_direct_endpoint_for_export(endpoint: PlanDocumentEndpoint) -> JsonObject:
    """Return one canonical direct endpoint export entry.

    Args:
        endpoint: Parsed direct endpoint selected in the plan.

    Returns:
        Canonical endpoint object with specification-owned selection metadata.
    """
    exported: JsonObject = {
        "method": endpoint.method,
        "path": endpoint.path,
        "required": endpoint.required is True,
        "locked": endpoint.locked is True,
    }
    if endpoint.operation_id is not None:
        exported["operationId"] = endpoint.operation_id
    return exported
